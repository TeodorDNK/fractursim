...
export async function POST(req: Request) {
  try {
    ...
    if (hp) return NextResponse.json({ ok: true }, { status: 200 }); // bot: ignorăm
    ...
    // Yahoo cere, de regulă, from = contul tău
    const info = await transporter.sendMail({
      from: `"Fracturism Site" <${SMTP_USER}>`,
      to: CONTACT_TO_EMAIL,
      replyTo: email, // ca să poți răspunde direct utilizatorului
