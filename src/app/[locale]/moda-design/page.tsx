...
export default async function FashionDesignPage({
  ...
  // Fallback colecții (înlocuiește cu datele tale / CMS)
  const collections: Collection[] =
    (t?.fashion?.collections as Collection[]) ?? [
      {
        slug: "capsula-ritm-fracturat",
        title: "Capsula • Ritm fracturat",
        blurb:
          "Silhuete curate, tăieturi asimetrice și imprimeuri care păstrează amprenta fisurii.",
        cover: "/images/ornament.png",
        season: "AW 2025",
      },
      {
        slug: "texturi-ceramica-textil",
        title: "Texturi • Ceramică & Textil",
        blurb:
          "Dialog între porozitatea ceramicii și moliciunea fibrelor. Suprafețe care respiră.",
        cover: "/images/separeu.png",
        season: "SS 2025",
      },
      {
        slug: "obiecte-pentru-interior",
        title: "Obiecte pentru interior",
        blurb:
          "Corpuri mici de lumină, tăvi, suporturi, toate compuse din motive recurente.",
        cover: "/images/moneda.png",
      },
    ];

  // Lookbook minimalist (poți înlocui cu imagini reale)
  const looks: Look[] = [
  ...
  // JSON-LD ca CollectionPage + hasPart (CreativeWork)
    ...
    <div className="min-h-screen w-full flex justify-center bg-[#1a404d] text-zinc-200">
      ...
      <div className="max-w-6xl w-full bg-transparent">
        ...
        <main className="px-6 pb-16 space-y-16">
          ...
          <section>
            <h2
              className="font-[var(--font-arhaic)] text-2xl mb-6"
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {collections.map((c) => (
                <article
                  key={c.slug}
                  className="rounded-xl border border-white/15 bg-black/10 backdrop-blur hover:bg-black/20 transition"
                >
                  <Link
                    href={`/${locale}/moda-design/${c.slug}`}
                  ...
                  >
                    ...
                    <div className="p-5">
                      ...
                      <h3
                        className="font-[var(--font-arhaic)] text-xl mt-1"
                        style={{ color: "var(--color-theme-accent)" }}
                      >
                        {c.title}
                      </h3>
                      <p className="mt-2 font-serif text-zinc-100 line-clamp-3">
                        {c.blurb}
                      </p>
                      <span className="mt-3 inline-block text-sm underline underline-offset-4 text-zinc-300">
                        Vezi colecția →
                      </span>
                    </div>
                  </Link>
          ...
          <section>
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {looks.map((l, i) => (
                <figure
                  key={i}
                  className="rounded-xl overflow-hidden border border-white/15 bg-black/10"
                >
          ...
          <section className="text-center">
            <p className="font-serif text-lg text-zinc-100">
              Cauți colaborare sau ediții speciale?
            </p>
            <Link
              href={`/${locale}/contact`}
              className="mt-4 inline-block px-6 py-3 rounded-lg border border-white/30 bg-black/30 hover:bg-black/50 text-white transition"
            >
              Scrie-ne →
            </Link>
          </section>
